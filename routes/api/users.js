...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const finChecker = await resEmail.then((value) => {
        return value
      })
    ...
    bcrypt.compare(password, user.passowrd).then((isMatch) => {
      if (isMatch) {
        if (confirmed !== true) {
          ...
          jwt.sign(payload, keys.secretOrKey, { expiresIn: 360000 }, (_err, token) => {
            res.json({
              sucess: true,
              token
            })
...
router.post('/recovery', async (req, res) => {
  try {
    ...
    const salty = await bcrypt.genSalt(10)
    const secretKey = await bcrypt.hash(email, salty)
    const newRecoveryToken = await bcrypt.hash(email, secretKey)
